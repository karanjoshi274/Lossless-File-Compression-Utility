...
void Huffman::huffer(unordered_map<char, int> frequencyMap) {
    ...
    while (HufferQueue.size() != 1) {
        leftNode = HufferQueue.top(); HufferQueue.pop();
        rightNode = HufferQueue.top(); HufferQueue.pop();
...
void Huffman::compressTofile(string InputfileName ,string OutputfileName) {
    ...
    while (inputStream.get(character)) {
        fileBits += codeMap[character];
    }
    inputStream.close();

    fileBits += codeMap[PSEUDO_EOF];

    // pad to multiple of 8
    int pad = (8 - (fileBits.size() % 8)) % 8;
    for (int i = 0; i < pad; i++) fileBits += '0';

    // write bytes
    for (size_t i = 0; i < fileBits.size(); i += 8) {
        bitset<8> bits(fileBits.substr(i, 8));
...
void Huffman::deHuffer(string compressedFileName, string decompressedFileName) {
    char character;
    string codeString;
    ifstream inputStream(compressedFileName, ios::in | ios::binary);
...
void Huffman::readHeader(ifstream &inputStream) {
    ...
    while (character != HEADER_TEXT_SEPERATOR && inputStream.good()) {
        if (character == CHARACTER_CODE_SEPERATOR) {
            inputStream.get(character);
            while (character != HEADER_ENTRY_SEPERATOR && inputStream.good()) {
...
void Huffman::decompressToFile(string codeString, Node *rootNode, string decompressedFileName) {
    ofstream outputStream(decompressedFileName, ios::out | ios::binary);
    ...
    for (size_t i = 0; i < codeString.size(); ++i) {
        ...
        if (traversingPointer->getCharacter() != INTERNAL_NODE_CHARACTER) {
            if (traversingPointer->getCharacter() == PSEUDO_EOF)
                break;
            outputStream.put(traversingPointer->getCharacter());
